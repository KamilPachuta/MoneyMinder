...
public sealed class EditMonthlyPaymentValidator : AbstractValidator<EditMonthlyPaymentRequest>
{
    ...
    {
        var result = await ValidateAsync(ValidationContext<EditMonthlyPaymentRequest>.CreateWithOptions((EditMonthlyPaymentRequest)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
